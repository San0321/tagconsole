...
router.post('/insert', function(req, res) {
    ...
    var db = req.db;
    // 
    db.collection('reserved').find().sort({
        'id': 1
    }).toArray(function(err, current) {
        if (err) {
            console.log(err);
        } else {
            ...
            db.collection('reserved').insertOne({
                "id": tagId,
                "name": tagName
            }, function(err, result) {
                if (err) {
...
router.post('/delete', function(req, res) {
    ...
    // for button click deletion
    if (req.body.id) {
        db.collection('reserved').deleteOne({
    ...
    if (itemsToBeDeleted) {
        ...
        for (var i = 0; i < itemsToBeDeleted.length; i++) {
            db.collection('reserved').deleteOne({
                "id": itemsToBeDeleted[i].toString()
            }, function(err, result) {
                if (err) {
                    console.log(err);
                    res.sendStatus(err);
